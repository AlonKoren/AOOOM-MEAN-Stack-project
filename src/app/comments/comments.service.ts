...
export class CommentsService {
  ...
  getFilteredComments(postId: string, commentsPerPage: number, currentPage: number) {
    const queryParams = `?pagesize=${commentsPerPage}&page=${currentPage}`;
    this.http.get<{ message: string, comments: any, maxComments: number }>(
      BACKEND_URL + 'post/' + postId + queryParams
    )
      .pipe(map((commentDate) => {
        return {
          comments: commentDate.comments.map(comment => {
  ...
  getComment(id: string) {
    return this.http.get<{ _id: string, content: string, creator: string, postId: string }>(
      BACKEND_URL + id
    );
  }
// add comment data
  ...
  updateComment(id: string, content: string, postId: string) {
    let commentDate: Comment | FormData;
    commentDate = new FormData();

    commentDate = { id, content, creator: null, postId: null, userName: null, commentDate: null };
